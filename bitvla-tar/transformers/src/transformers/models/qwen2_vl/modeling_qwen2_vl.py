...
def mistral_activation_quant(x, num_bits=8, sparse_blocksize=-1):
    def _sparse_quant_ste(x, num_bits, sparse_blocksize):
        dtype = x.dtype
        dim = x.shape[-1]
        x = x.float()
        if sparse_blocksize > 0.0:
            k = int((1 - sparse_blocksize) * dim)
    ...
    quantized_x = _sparse_quant_ste(x, num_bits, sparse_blocksize)
...
class Qwen2MLP(nn.Module):
    def __init__(self, config):
        ...
        self.sparse_ratio = getattr(config, "sparse_ratio", -1)
        print("mlp sparse_ratio", self.sparse_ratio)

    def forward(self, x):
        # return self.down_proj(self.act_fn(self.gate_proj(hidden_state)) * self.up_proj(hidden_state))
        x = mistral_activation_quant(x, num_bits=8, sparse_blocksize=self.sparse_ratio)
...
class Qwen2VLAttention(nn.Module):
    ...
    def __init__(self, config: Qwen2VLConfig, layer_idx: Optional[int] = None):
        ...
        self.sparse_ratio = getattr(config, "sparse_ratio", -1)
        print("attn sparse_ratio", self.sparse_ratio)
    ...
    def forward(
        ...
        hidden_states = mistral_activation_quant(hidden_states, num_bits=8, sparse_blocksize=self.sparse_ratio)
        ...
        attn_output = mistral_activation_quant(attn_output, num_bits=8, sparse_blocksize=self.sparse_ratio)
