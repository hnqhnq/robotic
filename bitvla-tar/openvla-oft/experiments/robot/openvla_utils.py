...
def update_auto_map(pretrained_checkpoint: str) -> None:
    ...
    with open(config_path, "r") as f:
        config = json.load(f)
    if "bitvla" in pretrained_checkpoint or "bitnet" in pretrained_checkpoint:
        config["auto_map"] = {
    ...
    print("Changes made:")
    if "bitvla" in pretrained_checkpoint or "bitnet" in pretrained_checkpoint:
        print('  - Set AutoConfig to "Bitvla_Config"')
        print('  - Set AutoModelForVision2Seq to "BitVLAForActionPrediction"')
    else:
        print('  - Set AutoConfig to "OpenVLAConfig"')
        print('  - Set AutoModelForVision2Seq to "OpenVLAForActionPrediction"')
...
def check_model_logic_mismatch(
    ...
    where_to_find_files_cur_codebase = "./prismatic/",
    ) -> None:
    ...
    for root, _, files in os.walk(where_to_find_files_cur_codebase):
...
def prepare_images_for_vla(images: List[np.ndarray], cfg: Any, image_size = OPENVLA_IMAGE_SIZE) -> List[Image.Image]:
    """
