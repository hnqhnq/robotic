...
class HFConvertConfig:
    ...
    output_hf_model_hub_path: str = "lxsy/openvla_wo_openx_pretrain"                # (Optional) Path to HF Hub Path to push
                                                                        # model to
...
def remap_state_dicts_for_hf(
    ...
    print("use_fused_vision_backbone", use_fused_vision_backbone)
    hf_state_dict = {}
    print("prismatic_vision_backbone_state_dict.keys()", prismatic_vision_backbone_state_dict.keys())
...
def convert_openvla_weights_to_hf(cfg: HFConvertConfig) -> None:
    ...
    for idx, timm_model_id in enumerate(hf_config.timm_model_ids):
        ...
        print("dix",idx)
        print("timm_model_id",timm_model_id)
        if "dino" in timm_model_id:
            prefix = "dino_featurizer."
        elif "siglip" in timm_model_id:
            prefix = "siglip_featurizer."
        for key, value in timm_vision_backbone.state_dict().items():
            timm_vision_backbones[f"{prefix}{key}"] = value
